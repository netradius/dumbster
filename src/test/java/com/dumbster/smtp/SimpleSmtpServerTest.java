...
public class SimpleSmtpServerTest {
	...
	public void testSendMessageWithCarriageReturn() {
		...
		try {
			sendMessage(SMTP_PORT, "sender@hereagain.com", "CRTest",
					bodyWithCR, "receivingagain@there.com");
		} catch (Exception e) {
			e.printStackTrace();
			fail("Unexpected exception: " + e);
		}
...
//		assertTrue(bodyWithCR.equals(email.getBody()));
	...
	public void testSendTwoMessagesSameConnection() {
		try {
			...
			mimeMessages[0] = createMessage(session, "sender@whatever.com",
					"receiver@home.com", "Doodle1", "Bug1");
			mimeMessages[1] = createMessage(session, "sender@whatever.com",
